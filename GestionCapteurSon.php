...
<head>
    <meta charset="UTF-8">
    <title>Admin – Capteur de son</title>
...
<div class="capteur-container">
  <h2>Gestion du capteur de son</h2>
    ...
    <div class="controls">
        <button class="btn" onclick="simulerLectureSon(false)">Lire le capteur</button>
        <button class="btn" onclick="toggleActionManuelle()">ON/OFF manuel</button>
        <label>Seuil : <input type="number" id="seuil" value="60"> dB</label>
    </div>
    ...
    <canvas id="graphiqueSon" height="120"></canvas>
...
<script>
    ...
    function boucleAuto() {
        if (document.getElementById('auto').checked && !actionManuel)
            simulerLectureSon(false);
    }

    function initGraph() {
        const ctx = document.getElementById('graphiqueSon').getContext('2d');
        chart = new Chart(ctx,{
            type:'line',
